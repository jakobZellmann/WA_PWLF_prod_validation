...
# PWLF
pwlf_raw <- read.csv('input/ValidationPWLF.csv')
...
# PWLF
pwlf <- pwlf_raw %>%
  mutate(iso3c = countrycode(as.numeric(substr(cntryID, 4, 6)), 'iso3n', 'iso3c')) %>%
  rename(year = Time,
         pop_tot_pwlf = Pop) %>%
  select(!c('cntryID', 'cntry'))
...
d <- d %>%
  ...
  mutate(Y_o_L = Y/pop_tot_wdde,
         ...
         PWLF_o_LF = PWLF/LF,
         Y_o_L_t_PWLF_o_LF = Y_o_L * PWLF_o_LF,
         log_Y_o_L_t_PWLF_o_LF = log(Y_o_L) * PWLF_o_LF) %>%
  
  
  ## select relevant variables
  select(c(iso3c, year, Y, L, pop_tot_wdde, pop_tot_wdde_g, Y_g, K_g, L_g, L_1_g, L_2_g, L_3_g, Y_o_L, log_Y_o_L, L_1_o_L, L_2_o_L, L_3_o_L, Y_o_L_t_L_1_o_L, Y_o_L_t_L_2_o_L, Y_o_L_t_L_3_o_L, log_Y_o_L_t_L_1_o_L, log_Y_o_L_t_L_2_o_L, log_Y_o_L_t_L_3_o_L, PWLF_g, PWLF_o_LF, Y_o_L_t_PWLF_o_LF, log_Y_o_L_t_PWLF_o_LF))
